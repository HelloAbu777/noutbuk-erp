        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <input type="date" value={to} onChange={e => setTo(e.target.value)}
              className="px-3 py-2 text-sm border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-blue-400" />
            <button onClick={load}
              className="px-4 py-2 text-sm border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
              Filter
            </button>
            <button onClick={() => setShowAdd(true)}
              className="flex items-center gap-2 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium rounded-lg ml-auto">
          ...
          {/* Summary */}
          {filtered.length > 0 && (
            <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4 flex gap-6 text-sm">
              ...
              <div><span className="text-gray-500">Jami summa: </span><span className="font-semibold text-gray-900 dark:text-white">{totalAmount.toLocaleString('uz-UZ')} so'm</span></div>
            </div>
          )}
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                {loading ? Array.from({ length: 5 }).map((_, i) => (
                  ...
                  <tr key={p._id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    ...
                    <td className="px-4 py-3 font-semibold text-gray-900 dark:text-white">{p.totalAmount.toLocaleString('uz-UZ')}</td>
                    <td className="px-4 py-3 text-green-600">{p.paidAmount.toLocaleString('uz-UZ')}</td>
                    <td className="px-4 py-3 text-gray-500 text-xs">{new Date(p.date).toLocaleDateString('uz-UZ')}</td>
                    <td className="px-4 py-3 text-gray-500 text-xs">{p.createdByName}</td>
                  </tr>
                ))}
              </tbody>
            </table>
