                  ...
                  <div key={p._id} className={`rounded-xl border p-3 transition-colors ${adding > 0 ? 'border-green-400 bg-green-50 dark:bg-green-500/10' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800'}`}>
                    ...
                    <div className="flex gap-2 text-xs mb-2">
                      <div className="flex-1 bg-gray-50 dark:bg-gray-900 rounded-lg p-1.5 text-center">
                        <p className="text-gray-400 text-xs">Do'kon</p>
                        <p className={`font-bold ${p.quantity === 0 ? 'text-red-500' : p.quantity < 5 ? 'text-amber-500' : 'text-green-600'}`}>{p.quantity}</p>
                      </div>
                      <div className="flex-1 bg-gray-50 dark:bg-gray-900 rounded-lg p-1.5 text-center">
                        <p className="text-gray-400 text-xs">Ombor</p>
                        <p className={`font-bold ${wQty === 0 ? 'text-gray-300 dark:text-gray-600' : 'text-blue-600 dark:text-blue-400'}`}>{wQty}</p>
                      </div>
                    </div>
                    <input type="number" min="0" placeholder="+0"
                      value={qtys[p._id] ?? ''}
